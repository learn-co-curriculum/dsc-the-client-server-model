...
In a web application, client requests are received by a web server known as an **HTTP (Hypertext Transfer Protocol) server**. The HTTP server is able to receive client requests by listening to one of the client's ports. Since clients have multiple ports, each port is identified using a port number that is always associated with the IP address of a computer. You can think of ports as separate channels on a computer that we can use to perform different tasks: one port could be surfing www.facebook.com while another fetches your email. This is possible because each application (the Web browser and the email client) uses different port numbers.
